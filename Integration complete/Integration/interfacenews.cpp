...
if (qry.exec())
    {   while (qry.next())
         ...
         ui->le_suppr->setText(qry.value(0).toString());
              ...
              ui->le_id_2->setText(qry.value(0).toString());
               ...
               /* QString date_string = qry.value(3).toString();
                ...
                QString datestring = ui->listView->model()->data(ui->listView->model()->index(index.row(),3)).toString();
                QDate date=QDate::fromString(datestring,"dd-MM-yy");
                ui->le_date_2->setDate(date);
         ...
         //    ui->le_categorie_2->findText(qry.value(4).toString());
                    ui->le_categorie_2->setCurrentText(ui->listView->model()->data(ui->listView->model()->index(index.row(),5)).toString());
...
void interfacenews::on_temp_ajout_clicked()  //ajout TEMPERATURE
{

       int temp=ui->templcdNumber->digitCount();

       //bool test=false;
       QSqlQuery query;
       QString temp_string= QString::number(temp);

       query.prepare("INSERT INTO TEMPERATURE (TEMPERATURE) "
                      "VALUES (:TEMPERATURE)");
       query.bindValue( 0 , temp_string);
...
/******************************** PARtie arduino ****************************************/
void interfacenews::rechEmp()
{
    data.append(AA.read_from_arduino());
      data= data.append(AA.read_from_arduino());
QString ch= data.append(AA.read_from_arduino());

     QSqlQueryModel *model=new QSqlQueryModel();

     model->setQuery("select * from EMPLOYES  where  ID = '"+ch+"'");
     QSqlRecord sr = model->record(0);
     QString code= sr.value(0).toString();

if (code!=""){
QString  message="ID : "+sr.value(0).toString()+"\nNom : "+sr.value(3).toString()+"\nPrenom : "+sr.value(4).toString();
     QMessageBox::information(this,"Employee existant  " , message ,QMessageBox::Ok);

     AA.write_to_arduino("1"); //envoyer 0 à arduino


}
else {
    QMessageBox::critical(nullptr, QObject::tr("Employee n'existe pas"),
             QObject::tr( data," n'existe pas.\n"
                        "click cancel to exit . ") , QMessageBox::Cancel);
...
void interfacenews::on_activer_moteur_2_clicked()
{ AA.write_to_arduino("1"); //envoyer 0 à arduino
...
void interfacenews::on_desactiver_moteur_clicked()
{
    AA.write_to_arduino("0"); //envoyer 0 à arduino
}
/***********************************************************/
...
void interfacenews::on_btnLogin_4_clicked()
{
    ...
    if (test)
    {
            while (query.next())
            {
                ...
                if (query.value(0).toString() == "Publicite" )
                {
                    msgBox.setText(" Connecté !");
                ...
                if (query.value(0).toString() == "Podcast" )
                {
                    msgBox.setText(" Connecté !");
...
void interfacenews::on_pb_modifier_4_clicked()
{


    //***
    QString titre=ui->l_titre_3->text();
    QString nom=ui->l_nom_3->text();
    QString nat=ui->l_nat_3->text();
    QString genre=ui->l_genre_3->text();
    QString descr=ui->l_descr_3->text();

     GESTION_MUSIQUE gm(titre,nom,nat,genre,descr);
    bool test=gm.modifier();
    QMessageBox msgbox;
...
void interfacenews::on_tab_musique_2_activated(const QModelIndex &index)
{


    QString titre=ui->tab_musique_2->model()->data(index).toString();




 QSqlQuery qry;
 qry.prepare("select * FROM GESTION_MUSIQUE WHERE TITRE='"+titre+"' or NOM='"+id+"' or NATIONALITE='"+id+"' or GENRE='"+id+"' or DESCRIPTION='"+id+"'" );
 ...
 if (qry.exec())
     {   while (qry.next())
         ...
         ui->l_titresupp_2->setText(qry.value(0).toString());
         ui->l_titre_3->setText(qry.value(0).toString());
        ui->l_nom_3->setText(qry.value(1).toString());
        ui->l_nat_3->setText(qry.value(2).toString());
        ui->l_genre_3->setText(qry.value(3).toString());
         ui->l_descr_3->setText(qry.value(4).toString());
