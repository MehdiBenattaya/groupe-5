...
//-------------------------------------------------------------------------------
 ...
 bool Actualite::modifier()
 {
     ...
     QSqlQuery query;
          query.prepare("UPDATE ACTUALITE set IDP='"+id+"',TITRE='"+Titre+"', LIEU='"+lieu+"', DATEA=:date, SOURCE='"+source+"', CATEGORIE='"+categorie+"', DESCRIPTION='"+description+"', IMAGE='"+imageurl+"' where IDP='"+id+"'");
           query.bindValue(":date" ,date);
          return query.exec();
 ...
 QSqlQuery query;
      ...
      query.prepare("SELECT *  FROM ACTUALITE where regexp_like(TITRE,:TITRE);");
